...
export const InternshipList = () => {
    ...
    const list = filteredInternships.map((d) => (
        <Card key={d._id} className="mb-3">
            <Card.Title style={{cursor: "pointer"}} onClick={() => gotoInternshipPost(d._id)}>
                {staredInternships.includes(d._id) ? "⭐ • " : null}
                {d.hasApplied ? "ANSØGT • " : null}
                {d.title}
            </Card.Title>
            <Card.Text style={{cursor: "pointer"}} onClick={() => gotoInternshipPost(d._id)}>
                {d.description.split(".")[0] + "."}
            </Card.Text>
            <p style={{cursor: "pointer"}} onClick={() => gotoInternshipPost(d._id)}>
            ...
            {(profileStore.User && profileStore.user.studentID) ?
                ...
                >
                    {staredInternships.includes(d._id) ? "Fjern fra favoriter" : "Tilføj til favoriter"}
                </p> : null}
        </Card>
    ))
